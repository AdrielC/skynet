...
object Shortcuts {
  ...
  def render[A: ToRefTree](value: sourcecode.Text[A]): Unit = {
    val id = value.source.filter(_.isLetter)
    val slide = Paths.get("slides", s"$id.png")
    if (!shownSlides(id) && slide.toFile.exists()) {
  ...
  def clear(): Unit = DotRenderer.render(
    Graph(true, true, None, Seq.empty), Paths.get("diagram.png"), RenderingOptions(), "png"
  )

  def generic[A](value: A)(implicit G: shapeless.LabelledGeneric[A]): G.Repr =
    G.to(value)
  ...
  private def tapRender[B: ToRefTree](value: B): B = { render(value); value }
